...
public:
    ...
    void markIsland(vector<vector<char>>&grid,char marker,Index index)
    {
        queue<Index>islandq;
        islandq.push(index);
        while(islandq.size()>0)
        {
            ...
            if(isValidIndex(grid,newIndex1) && grid[newIndex1.row][newIndex1.col]=='1')islandq.push(newIndex1);

            Index newIndex2;
            newIndex2.row=curIndex.row+1;
            newIndex2.col=curIndex.col;
            if(isValidIndex(grid,newIndex2)&& grid[newIndex2.row][newIndex2.col]=='1')islandq.push(newIndex2);

            Index newIndex3;
            newIndex3.row=curIndex.row;
            newIndex3.col=curIndex.col+1;
            if(isValidIndex(grid,newIndex3)&&grid[newIndex3.row][newIndex3.col]=='1')islandq.push(newIndex3);

            Index newIndex4;
            newIndex4.row=curIndex.row;
            newIndex4.col=curIndex.col-1;
            if(isValidIndex(grid,newIndex4)&& grid[newIndex4.row][newIndex4.col]=='1')islandq.push(newIndex4);
